...
absl::StatusOr<std::unique_ptr<PjRtBuffer>> TfrtGpuClient::BufferFromHostBuffer(
  ...
  bool should_use_staging_buffer = !IsDmaMapped(data, packed_size) &&
                                   should_stage_host_to_device_transfers() &&
                                   packed_size < (int64_t{1} << 30);

  bool use_staging_buffer =
      must_use_staging_buffer || should_use_staging_buffer;
...
PjRtFuture<> TfrtGpuBuffer::ToLiteral(MutableLiteralBase* literal) {
  ...
  if (on_device_shape_.IsTuple() && on_device_shape_.tuple_shapes().empty()) {
  ...
  EnqueueWorkWhenReady(
      ...
      [device(device_), device_buffer, usage_event(std::move(usage_event)),
       literal, promise, on_device_shape{on_device_shape_},
       transfer_manager]() mutable {
        ...
        ShapedBuffer shaped_buffer =
            device_buffer->buffer()->AsShapedBuffer(on_device_shape, device);
        ...
        transfer_manager->TransferLiteralFromDevice(
            stream.get(), shaped_buffer, literal,
