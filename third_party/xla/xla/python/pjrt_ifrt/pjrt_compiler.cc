...
absl::StatusOr<std::unique_ptr<LoadedExecutable>> PjRtCompiler::Compile(
  ...
  return PjRtLoadedExecutable::Create(
      ...
      std::move(xla_compile_options->sharding_devices));
}
...
PjRtCompiler::DeserializeLoadedExecutable(
  ...
  TF_ASSIGN_OR_RETURN(
      ...
      client_->pjrt_client()->LoadSerializedExecutable(
          ...
          xla::LoadOptions()));
  // TODO(emilyaf): Remove the else branch once sharding devices are plumbed
  // through from Australis and are always present in the
  // DeserializeExecutableOptions.
  DeviceListRef sharding_devices;
  if (xla_deserialize_options->sharding_devices.has_value()) {
    sharding_devices =
        std::move(xla_deserialize_options->sharding_devices.value());
  } else {
    xla::DeviceAssignment device_assignment =
    ...
    sharding_devices = client_->MakeDeviceList(devices);
  }
  return PjRtLoadedExecutable::Create(
      client_,
