...
PjRtLoadedExecutable::CreateInternal(
    ...
    std::vector<tsl::RCReference<LoadedHostCallback>> loaded_host_callbacks,
    DeviceListRef sharding_devices) {
  // For jit(pmap(...)), the device assignment (passed as `sharding_devices`)
  ...
  // `sharding_devices` with the executable's addressable devices if necessary.
  if (sharding_devices->devices().size() == 1) {
    if (pjrt_loaded_executable->addressable_devices().size() > 1) {
      ...
      sharding_devices = BasicDeviceList::Create(std::move(ds));
      ...
      if (ifrt_device != sharding_devices->devices().front()) {
        return FailedPrecondition(
            "Addressable device does not match sharding device");
      }
    }
  }
  if (sharding_devices->devices().size() <
  ...
  return std::unique_ptr<LoadedExecutable>(new PjRtLoadedExecutable(
      client, std::move(pjrt_loaded_executable), std::move(sharding_devices),
...
PjRtLoadedExecutable::Execute(absl::Span<tsl::RCReference<Array>> args,
  ...
  if (!pjrt_outputs.empty()) {
    num_outputs = pjrt_outputs.front().size();
